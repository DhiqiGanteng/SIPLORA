...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" href="../css/instruksi.css">
    <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.5.0/css/all.css" integrity="sha384-B4dIYHKNBt8Bc12p+WXckhzcICo0wtJAoU8YZTY5qE0Id1GSseTk6S+L3BlXeVIU" crossorigin="anonymous">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/malihu-custom-scrollbar-plugin/3.1.5/jquery.mCustomScrollbar.min.css">

    <title>Instruksi Laporan BDPB</title>
  </head>
  <body>
        ...
        <nav id="sidebar">
          ...
          <div class="col-md-12 list-rute">
            <div class="list-group">
               <a href="{{url('/berandaBDPB')}}" class="list-group-item list-group-item-action ">
               <i class="fas fa-tachometer-alt"></i>  Dashboard
               </a>
               <a href="{{url('/berandaBDPB/instruksiBDPB')}}" class="list-group-item list-group-item-action active btnActive"> <i class="fas fa-cog"></i> Instruksi Laporan</a>
               <a href="{{url('/berandaBDPB/validasiBDPB')}}" class="list-group-item list-group-item-action"> <i class="fas fa-check-square"></i> Validasi Laporan</a>
               <a href="{{url('/berandaBDPB/rekapitulasiBDPB')}}" class="list-group-item list-group-item-action"> <i class="fas fa-clipboard-check"></i> Rekapitulasi</a>
             </div>
          </div>
      ...
      <nav class="navbar navbar-expand-lg navbar-light bg-light">
          <div class="container-fluid">
            <div class="row">
              ...
              <h4>Instruksi Admin BDPB {{Session::get('daerah')}}</h4>
            </div>
                   ...
                   <tbody>
                    @foreach($data as $laporan)
                     <tr>
                       ...
                       <td><img src="{{ asset('gambar/'.$laporan->foto)  }}"  style="width:50px;height:50px"></td>
                       <td><div class="row">
                                      <!-- Button trigger modal -->
                            <button type="button" data-jenis ="{{ $laporan->jenis_bencana }}" data-kodelaporan ="{{$laporan->kode_laporan}}" data-tanggal ="{{ $laporan->updated_at }}" data-deskripsi ="{{ $laporan->deskripsi }}" data-lokasi ="{{$laporan->get_province['name']}}, {{$laporan->get_regency['name']}}, {{$laporan->get_district['name']}},{{$laporan->get_village['name']}}" data-namapelapor ="{{ $laporan->get_pelapor['nama'] }}" data-nohp ="{{ $laporan->get_pelapor['nohp'] }}" data-email ="{{ $laporan->get_pelapor['email'] }}" data-pemerintah ="{{ $laporan->get_pemerintah['nama'] }}" data-foto ="{{ asset('gambar/'.$laporan->foto)  }}" data-status ="{{ $laporan->status }}"class="btn btn-outline-primary detail" data-toggle="modal" data-target="#exampleModalCenter" ><i class="fas fa-info-circle"></i></button>
                         <form method="post" action="/berandaBDPB/instruksiBDPB/diterima/{{ $laporan->id }}" >
                           {{ csrf_field() }}
                               {{ method_field('PATCH') }}
                            <button type="submit" class="btn btn-outline-success instruksiBTN"><i class="fas fa-arrow-alt-circle-right"></i></button>
                            </form>
                            <form method="POST" action="/berandaBDPB/instruksiBDPB/ditolak/{{ $laporan->id }}" >
                             {{ csrf_field() }}
                                 {{ method_field('PATCH') }}
                            <button type="submit"  class="btn btn-outline-danger tolak2"><i class="fas fa-window-close tolak"></i></button>
                            </form>
                          </div>
                       </td>
                     </tr>
...
$('#haha').DataTable();
  ...
  <script>
      ...
      $('#exampleModalCenter').on('show.bs.modal', function (event){
          ...
          var showgambar = document.getElementById("showgambar");
